...
Protected Class Win32Thread
	#tag Method, Flags = &h0
		Sub Constructor()
		  ...
		  mLocalStorage = Win32.Kernel32.TlsAlloc()
	...
	#tag Method, Flags = &h21
		Private Shared Function DefThreadProc(Parameter As Ptr) As Integer
		  ...
		  While Not ThreadProcLock.TryEnter
		    Win32.Kernel32.SleepCurrentThread(100)
		  Wend
		  Try
	...
	#tag Method, Flags = &h0
		Sub Kill(ExitCode As Integer = 0)
		  Call Win32.Kernel32.TerminateThread(Me.Handle, ExitCode)
		  mLastError = Win32.LastError
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub Resume()
		  ...
		  Call Win32.Kernel32.ResumeThread(Me.Handle)
		  mLastError = Win32.LastError
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub Run()
		  ...
		  mHandle = Win32.Kernel32.CreateThread(Nil, 0, AddressOf DefThreadProc, Ptr(token), CREATE_SUSPENDED, mThreadID)
		  mLastError = Win32.LastError
		  If Me.Handle > 0 Then
		    ...
		    Do
		      count = Win32.Kernel32.ResumeThread(Me.Handle)
		      mLastError = Win32.LastError
		    Loop Until count <= 0
		  End If
		End Sub
	...
	#tag Method, Flags = &h0
		Sub Sleep(Milliseconds As Integer)
		  ...
		  Win32.Kernel32.SleepCurrentThread(Milliseconds)
		  mLastError = Win32.LastError
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub Suspend()
		  ...
		  Call Win32.Kernel32.SuspendThread(Me.Handle)
		  mLastError = Win32.LastError
		End Sub
	#tag EndMethod
	...
	#tag ComputedProperty, Flags = &h0
		#tag Getter
			Get
			  Return Win32.Kernel32.TlsGetValue(mLocalStorage)
			End Get
		#tag EndGetter
		#tag Setter
			Set
			  Call Win32.Kernel32.TlsSetValue(mLocalStorage, value)
			End Set
		#tag EndSetter
		LocalStorage As Ptr
