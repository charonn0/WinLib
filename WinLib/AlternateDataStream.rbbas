...
Protected Class AlternateDataStream
	...
	#tag Method, Flags = &h0
		Function Count() As Integer
		  ...
		  #If TargetWin32 Then
		    If KernelVersion >= 5.0 And KernelVersion < 6.0 Then
		      
		      Dim fHandle As Integer = WinLib.Kernel32.CreateFile(MainStream.AbsolutePath, 0,  FILE_SHARE_READ Or FILE_SHARE_WRITE, 0, OPEN_EXISTING, 0, 0)
		      If fHandle > 0 Then
		      ...
		      Else
		        Return -1
		      End If
		    ElseIf KernelVersion >= 6.0 Then
		      Dim buffer As WIN32_FIND_STREAM_DATA
		      Dim sHandle As Integer = WinLib.Kernel32.FindFirstStream(MainStream.AbsolutePath, 0, buffer, 0)
		      Dim ret As Integer
		...
		Function Delete(StreamName As String) As Boolean
		  ...
		  #If TargetWin32 Then
		    If MainStream <> Nil Then
		      Return WinLib.Kernel32.DeleteFile(MainStream.AbsolutePath + ":" + StreamName + ":$DATA")
		    Else
		      Return False
		    End If
		  #endif
	...
	#tag Method, Flags = &h0
		Function Stream(StreamIndex As Integer) As String
		  ...
		  #If TargetWin32 Then
		    ...
		    If KernelVersion < 6.0 And KernelVersion >= 5.0 Then
		      Dim fHandle As Integer = WinLib.Kernel32.CreateFile(MainStream.AbsolutePath, 0,  FILE_SHARE_READ Or FILE_SHARE_WRITE, 0, OPEN_EXISTING, 0, 0)
		      If fHandle > 0 Then
		        Dim mb As New MemoryBlock(64 * 1024)
		        Dim status As IO_STATUS_BLOCK
	...
	#tag Method, Flags = &h0
		Function Stream(StreamName As String, CreateNew As Boolean = False) As FolderItem
		  ...
		  #If TargetWin32 Then
		    ...
		    Dim fHandle As Integer = WinLib.Kernel32.CreateFile(MainStream.AbsolutePath + ":" + StreamName + ":$DATA", 0, _
		    FILE_SHARE_READ Or FILE_SHARE_WRITE, 0, CREATE_NEW, 0, 0)
		    If fHandle > 0 Then
		      target = GetFolderItem(MainStream.AbsolutePath + ":" + StreamName + ":$DATA")
		      Call WinLib.Kernel32.CloseHandle(fHandle)
		      Return target
		    Else
		      If WinLib.Kernel32.GetLastError = 80 And CreateNew Then  //ERROR_FILE_EXISTS
		        target = GetFolderItem(MainStream.AbsolutePath + ":" + StreamName + ":$DATA")
		        Return target
		      End If
		    End If
