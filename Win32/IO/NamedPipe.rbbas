...
Inherits Win32.IO.Stream
	...
	#tag Method, Flags = &h0
		Sub Connect()
		  ...
		  End If
		  #If TargetWin32 Then
		    Dim hFile As Integer = Win32.Libs.Kernel32.CreateFile("\\.\pipe\" + PipeName, GENERIC_READ Or GENERIC_WRITE, 0, Nil, OPEN_EXISTING, 0, 0)
		    If hFile > 0 Then
		      Me.mHandle = hFile
		      mIsConnected = True
	...
	#tag Method, Flags = &h0
		Sub Listen(OpenMode As Integer, MaxInstances As Integer = PIPE_UNLIMITED_INSTANCES, OutBufferSize As Integer = 512, InBufferSize As Integer = 512, DefaultTimeout As Integer = - 1, PipeMode As Integer = 0)
		  ...
		  Dim err As Integer
		  Dim hFile As Integer = Win32.Libs.Kernel32.CreateNamedPipe("\\.\pipe\" + PipeName, OpenMode Or FILE_FLAG_OVERLAPPED, PipeMode, _
		  ...
		  If err = 0 Then
		    ...
		    over.Offset = 0
		    over.hEvent = 0
		    If Win32.Libs.Kernel32.ConnectNamedPipe(hFile, over) Then
		      Me.mHandle = hFile
		    Else
		      err = Win32.LastError
