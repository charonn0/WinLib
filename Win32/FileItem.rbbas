...
Inherits FolderItem
	...
	#tag Method, Flags = &h0
		Function ADS(StreamName As String, CreateNew As Boolean = False) As FileItem
	...
	#tag Method, Flags = &h0
		Function HardLink(Index As Integer) As FileItem
		  //Returns the hardlink at Index. Windows Vista and newer only. The index may change at any time.
		  //Use this method in a loop with HardLinkCount
		  ...
		  #If TargetWin32 Then
		    ...
		    If findHandle <> INVALID_HANDLE_VALUE Then
		      Do
		        ret = New FileItem(GetFolderItem(Left(Self.AbsolutePath, 2) + linkname.WString(0)))
		        buffSize = linkname.Size
	...
	#tag Method, Flags = &h0
		Function Lock(Offset As UInt64 = 0, Length As UInt64 = -1) As Boolean
		  ...
		  #If TargetWin32 Then
		    ...
		    If LockHandle > 0 Then
		      Dim startLo, startHi, stopLo, stopHi As Integer
		      startLo = BitAnd(Offset, &hFFFFFFFF)
		      startHi = ShiftRight(Offset, 32)
		      If Length = -1 Then Length = Me.Length
		      stopLo = BitAnd(Length, &hFFFFFFFF)
		      stopHi = ShiftRight(Length, 32)
		      Return WinLib.Kernel32.LockFile(LockHandle, startLo, startHi, stopLo, stopHi)
		    End If
		  #endif
		End Function
	#tag EndMethod
